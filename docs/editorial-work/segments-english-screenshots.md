...
- `git diff --check` passes and the staged diff contains only this visual completion batch. Pending: local commits and progress reconciliation, Help PR and checks, merge commit, normal Netlify deployment, public English and Spanish page verification. Do not mark the new English captures published until those complete.
