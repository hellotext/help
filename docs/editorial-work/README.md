...
Use `visual_pending` when bilingual text and the local build/browser checks passed, but the article's own plan identifies a useful product capture that was deferred. The `local_verified_commit` field then retains the verified text commit; it does not claim that the capture exists. Resume the screenshot task from the linked work record without repeating the completed prose review. Move the row to `local_verified` only after the image and complete bilingual pages pass the shared screenshot and browser checks. The current queue and the deliberate text-only decisions are in `visual-followup.md`.
