...
| After the **Y/O** explanation | How do two alternative conditions and a required block relate? | `images/audience/segments/rule-logic-es.png`, 1144 × 1250. | Real unsaved editor with two activity alternatives under **O** and an age characteristic under **Y**. |
...
- All three published assets are genuine PNGs with embedded Display P3 profiles and at least 2.08 source pixels per planned maximum display CSS pixel. The menu copy is byte-identical to its original. The two editor derivatives are lossless crops without resampling. Capture records and untouched originals are under `captures/segments/` and excluded from the public site.
...
- The Spanish article now contains the three figures, alt text, accessible hidden captions, and full-size links. The English article has a matching text example without a Spanish-language screenshot.
...
- Browser review at 1635px desktop and 390px mobile passed for the Spanish guide. All three figures appear in the intended order, full-size links open the PNGs at their native dimensions, alt text and hidden captions are present, and the page has no horizontal overflow. The English guide remains readable at 390px with no overflow. The rule screenshot's individual labels are small inline on mobile; the full-size link is available for detail. Chrome reported no application errors. The plain local static server needs `.html` URLs and does not emulate production's extensionless rewrite.
...
- Publication: original pages remain live. This branch has local pilot work only; no merge or deployment has been made.
