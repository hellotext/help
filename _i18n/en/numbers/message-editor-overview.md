Use the message editor whenever you write content for Hellotext conversations, campaigns, journeys, and playbooks.

The editor helps you create messages that are clear, trackable, and personalized for each customer profile.

## What you can do

Use the editor to:

- Format text with bold, italic, and links.
- Add tracked links to measure clicks and activity.
- Insert personalization tags based on customer profile properties.
- Share supported rich content, such as locations when the channel allows it.

## Where the editor appears

You may see the editor in places such as:

- The inbox, when replying to a customer.
- Campaigns and broadcasts, when creating a one-time send.
- Journeys and playbooks, when writing automated messages.

Available tools can vary by channel. For example, SMS supports text and links, while WhatsApp can support richer message types such as locations.

## Related guides

- [Message editor basics]({% link _numbers/message-editor-basics.md %})
- [Tracked links]({% link _analytics-reporting-attribution/tracked-links.md %})
- [Personalization tags]({% link _audience/personalization-tags.md %})
- [Share a location]({% link _numbers/share-a-location.md %})
