...
For product discovery specifically, see [Smart Recommender playbook]({% link _journeys/smart-recommender-playbook.md %}). For order-status questions, see [Order-Update Delight playbook]({% link _journeys/order-update-playbook.md %}). For frequent support questions, see [Instant Answers playbook]({% link _journeys/instant-answers-playbook.md %}). For guided return or exchange support, see [Return & Exchange Helper playbook]({% link _journeys/return-and-exchange-helper-playbook.md %}). For a custom reactive agent with your own intents, prompt, knowledge, channels, tone, and handoff, see [Custom Agent playbook]({% link _journeys/custom-agent-playbook.md %}).
