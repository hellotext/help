---
navigation_group: foundations
languages: ["en", "es"]

en:
  title: Setup overview
  description: Choose the right setup steps before launching channels, captures, playbooks, routes, or campaigns.
es:
  title: Resumen de configuración
  description: Elige los pasos de configuración correctos antes de lanzar canales, capturas, misiones, rutas o campañas.

permalink: setup-overview
permalink_es: resumen-configuracion

layout: guide
topic: integrations
popular: false
---

{% translate_file integrations/setup-overview.md %}
